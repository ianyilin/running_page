...
const Header = () => {
  ...
  return (
    <>
      <nav className="rp-header mx-auto flex w-[calc(100%-2rem)] min-w-0 max-w-screen-2xl items-center justify-between px-4 py-3 lg:px-5">
        <div className="flex min-w-0 items-center gap-3">
          <Link to={siteUrl}>
            <picture>
              <img
                className="rp-logo h-12 w-12 rounded-full"
                alt="logo"
                src={logo}
              />
            </picture>
          </Link>
          <span className="rp-header-title hidden truncate font-semibold sm:block">
            Lin Yi Running
          </span>
        </div>
        <div className="flex min-w-0 items-center justify-end text-right">
          {navLinks.map((n, i) => (
            <a
              key={i}
              href={n.url}
              className="rp-nav-link mr-2 text-sm lg:mr-3"
            >
              {n.name}
            </a>
          ))}
